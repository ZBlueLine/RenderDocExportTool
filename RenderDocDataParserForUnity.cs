...
namespace Windsmoon.Tools
{
    public static class RenderDocDataParserForUnity
    {
        ...
        public static void ParseMeshData()
        {
            ...
            ParseMesh(path, stringDataList);
        }

        [MenuItem("Windsmoon/Tools/Parse All CSV Mesh Data In Folder")]
        public static void ParseAllMeshData()
        {
            List<StreamReader> sr;
            string[] path = LoadAllCSV(out sr);
            for(int i = 0; i < sr.Count; i++)
            {
                sr[i].ReadLine(); // pass the title
                List<string> stringDataList = new List<string>();

                while (!sr[i].EndOfStream)
                {
                    string tempData = sr[i].ReadLine();
                    tempData = tempData.Replace(" ", "");
                    tempData.Replace("\r", "");
                    tempData.Replace("\n", "");
                    stringDataList.Add(tempData);
                ...
                sr[i].Close();
                ParseMesh(path[i], stringDataList);
            }
        }

        private static void ParseMesh(string path, List<string> stringDataList)
        {
            ...
            string relativeDirectoryPath = directoryPath.Replace("\\", "/");
            relativeDirectoryPath = relativeDirectoryPath.Replace(Application.dataPath, "Assets");
            Debug.Log(relativeDirectoryPath + "/" + System.IO.Path.GetFileNameWithoutExtension(path) + "_" + System.DateTime.Now.Ticks + ".mesh");
            AssetDatabase.CreateAsset(mesh, relativeDirectoryPath + "/" + System.IO.Path.GetFileNameWithoutExtension(path) + "_" + System.DateTime.Now.Ticks + ".mesh");
        ...
        private static string[] LoadAllCSV(out List<StreamReader> sr)
        {
            string folderPath = EditorUtility.OpenFolderPanel("select csv folser", String.Empty, "csv");
            string[] csvPaths = Directory.GetFiles(folderPath, "*.csv", SearchOption.AllDirectories);

            sr = new List<StreamReader>();
            foreach (string csvPath in csvPaths)
            {
                Debug.Log(csvPath);
                sr.Add(new StreamReader(new FileStream(csvPath, FileMode.Open)));
            }
            return csvPaths;
        }
        #endregion
